					...
					단,커맨트 객체명은 소문자로 시작
			         --> 
         				<span style="color: red; font-size: .8em">
         					${nameError}<form:errors path="formCommand.name" />
         				</span>
         			</td>
				</tr>
				<tr bgcolor="white">
					...
					<td><input type="text" name="years" value="${param.years}" />
						<span style="color: red; font-size: .8em">
							${yearsError}<form:errors path="formCommand.years" />
						</span>
					</td>
				</tr>
				<tr bgcolor="white">
					<td>성별</td>
					<td><input type="radio" name="gender" value="남자" <c:if test="${param.gender=='남자' }">checked="checked"</c:if> />남자
