...
<body>
	<fieldset>
		...
		<hr />
		<c:if test="${user !=null }">
			<span style="color: red; font-size: 2.5em; font-weight: bold">아이디:${sessionScope.user },비번:${sessionScope.pass}</span>
		</c:if>
		<c:if test="${not empty message }">
			<br />
			<span style="color: green; font-size: 2.5em; font-weight: bold">${message}</span>
		</c:if>
		<fieldset>
		...
		<c:if test="${not empty name }">
			<br />
			<span style="color: green; font-size: 2.5em; font-weight: bold">이름:${name},10년후 나이:${years}</span>
		</c:if>
		<fieldset>
			...
			<c:if test="${nick !=null }">
				<span style="color: green; font-size: 2.5em; font-weight: bold">이름:${nick },나이:${age },관심사항:${inters },성별:${gender }</span>
			</c:if>
		...
		<fieldset>
			...
			<form action="<c:url value='/Annotation/ModelAttribute.do'/>" method="get">
				<table bgcolor="gray" cellspacing="1" width="60%">
					...
					<tr bgcolor="white">
						<td>자기소개</td>
							<td><textarea name="self" cols="30" rows="10"></textarea>
						</td>
					</tr>
		...
		<fieldset>
			...
			<c:if test="${not isNotLogin }"> ${sessionScope.loginCommand.user}님 즐감하세요!!!<br/>
			${sessionScope.user}님 즐감하세요!!!
			<a href="<c:url value="/Annotation/SessionAttributeLogout.do"/>">로그아웃</a>
			</c:if>
			<br /> <a href="<c:url value="/Annotation/isLogin.do"/>">로그인 여부 판단 : ${isLoginMessage}</a>
			<h4>
